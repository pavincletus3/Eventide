...
import { CalendarDays, LogIn, LogOut, UserPlus, PlusCircle, Loader2, UserCog, Users } from 'lucide-react';
...
const Header = () => {
  ...
  const canCreateEvent = userRole === 'organizer' || userRole === 'coadmin' || userRole === 'admin';
    ...
    <header className="bg-card border-b border-border shadow-sm">
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1 sm:gap-2">
          {(authStateLoading || (user && isRoleLoading)) && !authOperationLoading ? (
            <Loader2 className="h-5 w-5 animate-spin text-primary" />
          ) : user ? (
            <>
              <span className="text-sm text-foreground hidden sm:inline mr-2">
                Welcome, {user.email?.split('@')[0]}
                {userRole && <span className="text-xs text-muted-foreground ml-1">({userRole})</span>}
              </span>
              {canCreateEvent && (
                <Button variant="ghost" size="sm" asChild>
                  <Link href="/events/create" title="Create Event">
                    <PlusCircle className="mr-1 h-4 w-4 sm:mr-1" /> <span className="hidden sm:inline">Create Event</span>
                  </Link>
                </Button>
              )}
              {isAdmin && (
                 <Button variant="ghost" size="sm" asChild>
                  <Link href="/admin/users" title="Manage Users">
                    <Users className="mr-1 h-4 w-4 sm:mr-1" /> <span className="hidden sm:inline">Users</span>
                  </Link>
                </Button>
              )}
