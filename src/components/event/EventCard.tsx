...
const EventCard: React.FC<EventCardProps> = ({ event }) => {
  const { id, name, description, date, venue, imageUrl } = event;
  ...
  if (date) {
    try {
      ...
      if (typeof eventDate.toDate === 'function') {
        ...
        formattedDate = format(eventDate.toDate(), "eee, MMM d, yyyy 'at' h:mm a");
      } else if (eventDate.seconds !== undefined && typeof eventDate.seconds === 'number') {
        // Plain object from server component serialization of Timestamp
        const jsDate = new Date(eventDate.seconds * 1000 + (eventDate.nanoseconds || 0) / 1000000);
        formattedDate = format(jsDate, "eee, MMM d, yyyy 'at' h:mm a");
      } else if (typeof eventDate === 'string' || typeof eventDate === 'number') {
        // Fallback for ISO string or numeric timestamp
        formattedDate = format(new Date(eventDate as string | number), "eee, MMM d, yyyy 'at' h:mm a");
      } else {
         // Fallback if it's already a Date object (less likely for Firestore Timestamp)
         formattedDate = format(eventDate as unknown as Date, "eee, MMM d, yyyy 'at' h:mm a");
      }
    } catch (error) {
  ...
  return (
    <Card className="flex flex-col h-full shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out overflow-hidden group">
      ...
      <CardContent className="p-4 flex flex-col flex-grow">
        <CardTitle className="text-xl font-semibold mb-2 text-primary line-clamp-2 h-[3.75rem]">{name || "Event Name Not Available"}</CardTitle> {/* Approx 2 lines height */}
        ...
        <p className="text-sm text-foreground mb-4 line-clamp-3 flex-grow min-h-[4.5rem]"> {/* Approx 3 lines height */}
